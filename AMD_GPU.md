...
(now upstream's `CUDA_RELEASE_HOST`; see the historical note in §7). Upstreamed as
[JustVugg/colibri#112](https://github.com/JustVugg/colibri/pull/112);
...
## 7. VRAM as additional pin capacity (now: `CUDA_RELEASE_HOST=1`)

> **Historical note:** this fork originally implemented this as `CUDA_EXTEND=1`.
> Upstream independently landed the same capability as `PIN=auto` + `PIN_FILL` +
> `CUDA_RELEASE_HOST` with deeper engine integration (including
> `expert_host_ensure` rematerialization), so `CUDA_EXTEND` was retired in the
> 54cfe56 sync. The description below is kept for the design rationale; use
> `CUDA_RELEASE_HOST=1` and see `GPU_BACKENDS.md` for current semantics.
