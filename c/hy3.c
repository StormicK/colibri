...
/* PILOT_REAL: cross-layer speculative expert prefetch (ported from glm.c). g_pilot_real
 * gates whether the background pilot thread's loads are REAL (published into ecache[L+1])
 * or just WILLNEED hints; g_cur_moe_layer/g_pilot_mx/g_pilot_cv are the handshake that stops
 * a speculative load from racing the main thread once it reaches that layer. */
static int g_pilot=0;            /* PILOT=1: router-guided prefetch enabled */
static int g_pilot_k=8;          /* PILOT_K=k: only the first k router predictions per position */
static int g_pilot_real=0;       /* PILOT_REAL=1: pilot issues REAL loads (default OFF, opt-in) */
static pthread_mutex_t g_pilot_mx=PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t g_pilot_cv=PTHREAD_COND_INITIALIZER;
static _Atomic int g_cur_moe_layer=-1;   /* highest layer the MAIN thread has entered moe() for */
static int g_pilot_inflight[256];        /* protected by g_pilot_mx */
    ...
    for(;;){
        ...
        for(;;){
            ...
            if(atomic_compare_exchange_weak_explicit(&p->cur,&c,c+1,memory_order_acq_rel,memory_order_relaxed)){
                ...
                expert_load(p->m,L,eid,&p->m->ws[i],1);
...
/* PILOT: router-guided cross-layer prefetch (ported from glm.c). Predicts the NEXT
 * sparse layer's top-K experts from the CURRENT layer's post-attention (stale) state
 * and hands them to a background thread, which either issues a lightweight WILLNEED
 * (default) or, with PILOT_REAL=1, a REAL disk load published into ecache[layer+1] --
 * overlapping disk I/O across layer boundaries instead of only within one layer's own
 * cache-miss batch (PIPE). Ring is lock-free 1P/1C; full = drop (a lost hint is not an
 * error). PILOT_REAL correctness invariant: the pilot only ever writes ecache[L] for
 * L > g_cur_moe_layer; moe() claims g_cur_moe_layer=layer and waits out any in-flight
 * pilot load for that layer before touching ecache[layer], so the two never race. */
static struct { int l,e; } pilot_q[4096];
static volatile unsigned pilot_w=0, pilot_r=0;
static Model *pilot_m=NULL;

/* PILOT_REAL: real load of the predicted expert into the FUTURE layer's LRU. The
 * (slow) pread runs OUTSIDE the lock, overlapped with the main thread's compute on the
 * current layer; the lock only protects slot selection/publication and the handshake. */
static void pilot_realload(Model *m, int layer, int eid){
    ...
    int slot,isnew;                                     /* grow if there's room, else evict LRU */
    ...
    int rc=expert_load(m,layer,eid,dst,0);              /* fatal=0: a speculative I/O error must not kill the server */

    pthread_mutex_lock(&g_pilot_mx);
    if(rc>=0){
        dst->used=(uint64_t)__atomic_add_fetch(&m->eclock,1,__ATOMIC_RELAXED);
        if(isnew) m->ecn[layer]=slot+1;                 /* publish the slot only now eid is valid */
        atomic_fetch_add_explicit(&g_pilot_loads,1,memory_order_relaxed);
    } else {
        atomic_fetch_add_explicit(&g_pilot_drops,1,memory_order_relaxed); /* load failed: slot stays hidden (eid=-1) */
    }
    g_pilot_inflight[layer]--;
    pthread_cond_broadcast(&g_pilot_cv);
    pthread_mutex_unlock(&g_pilot_mx);
    if(rc<0)                                            /* never swallow silently: log once and move on */
        fprintf(stderr,"[PILOT] speculative load dropped: layer %d expert %d (I/O error/short read) -- no impact on output\n",layer,eid);
}
...
/* Predicts lnext's top-K router picks from x (the CURRENT layer's post-attention, stale
 * state -- lnext's own attention hasn't run yet, this is a cheap approximation, not exact)
 * and enqueues the ones not already resident for the pilot thread to fetch ahead of time. */
static void pilot_prefetch(Model *m, int lnext, const float *x, int S){
    ...
    for(int s=0;s<S;s++){
        ...
        for(int kk=0;kk<K;kk++){
            ...
            ch[best]=-2e30f;
            /* Residency scan of the FUTURE layer lnext under g_pilot_mx: with PILOT_REAL=1
             * the pilot worker mutates ecache[lnext]/ecn[lnext] concurrently, so read them
             * under the same lock. Decide under the lock, then enqueue AFTER unlocking --
             * pilot_q is lock-free (pilot_w/pilot_r atomics, not g_pilot_mx) so there's no
             * re-entrant double-lock, and the worker re-checks residency anyway, making a
             * racing redundant enqueue harmless. */
            int found=0;
            pthread_mutex_lock(&g_pilot_mx);
            ESlot *P=m->pin[lnext];
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    if(g_pilot_real){   /* cross-layer barrier: claim this layer and wait out any in-flight
                         * pilot load for it, so ecache[layer] is stable for the rest of moe(). */
        pthread_mutex_lock(&g_pilot_mx);
...
static void layers_forward(Model *m, float *x, int S, int pos_base){
    Cfg *c=&m->c; int D=c->hidden;
    if(g_pilot_real){   /* new forward pass: layer ownership restarts at -1 (layers redo from 0) */
        pthread_mutex_lock(&g_pilot_mx);
...
#endif
        double t0=now_s();
        expert_load(m,cd[b].l,cd[b].eid,s,1);
        const char *tier="RAM";
#ifdef COLI_CUDA
        if(gpu){
            if(qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)){
...
static void perf_report(Model *m){
    ...
    if(g_pilot_real) fprintf(stderr,"[PILOT_REAL] %ld cross-layer loads completed, %ld dropped (main already on layer) | PILOT_K=%d\n",
 ...
 * and again after every completed request (ported from glm.c). Without this
 * the dashboard's RAM/VRAM header never receives a payload (Engine.hwinfo
 * stays None forever), which is why it showed wrong/missing numbers for hy3
 * even though TIERS (below) was already ported and working. */
static void hwinfo_emit(Model *m){
    ...
    if(g_cuda_ndev>0) snprintf(gpu_name,sizeof(gpu_name),"CUDA device x%d",g_cuda_ndev);
#endif
...
/* TIERS: expert-tier snapshot -- "TIERS <vram> <ram> <disk> <vram_gb> <ram_gb>"
 * on the protocol channel. 'coli' (cmd_chat) unconditionally reads one line
 * right after READY/STAT for every engine (same wire format as glm.c's
 * tiers_emit); skip it and the first byte of the next answer gets eaten by
 * cmd_chat's leftover readline(), or -- if nothing is ever written -- the
 * readline() blocks forever waiting for a line that will never come. */
...
/* EMAP/HITS: per-expert tier+heat map for the web dashboard's "Brain" tab
 * (ported from glm.c). g_ehit records, per layer/expert, whether it was used
 * at all since the last hits_emit() call; ehit_mark() is called from the
 * same per-token top-k selection loop that already increments m->eusage. */
 ...
 * rows = sparse layers in order (+MTP if present), cols = n_experts. Hex. */
...
/* HITS: 1bit/expert bitmap (same order as EMAP), reset for the next turn after emit. */
...
/* ---- SERVE_BATCH: OpenAI-API/dashboard wire protocol (ported from glm.c) ----
 * openai_server.py's Engine writes "SUBMIT <id> <slot> <bytes> <max_tokens>
 * <temp> <top_p>\n" + raw payload + "\n" and expects streamed "DATA <id> <n>\n"
 * + n bytes + "\n" per token, then "DONE <id> STAT ..." at the end (glm.c's
 * ServeCtx/ServeReq/mux_submit run several such conversations concurrently,
 * one KV slot each, with continuous batching across slots at decode time).
 * hy3 has no multi-slot KV abstraction (kv_alloc() owns one cache embedded in
 * Model) -- porting genuine cross-slot batching would mean rebuilding hy3's
 * whole KV cache as a swappable per-context struct. Instead this supports a
 * single conversation (slot 0) processed synchronously end-to-end per
 * request via the existing spec_decode() generator; a second concurrent
 * SUBMIT gets ERROR ... SLOT_BUSY same as glm.c would for an exhausted slot
 * pool, and KV_SLOTS>1 is rejected at startup. Good enough for one agent
 * driving one conversation at a time against the API; extend later if
 * multi-session concurrency against hy3 is actually needed. */
...
typedef struct { Tok *T; unsigned long long id; } EmitMux;
static void emit_mux(int t, void *ud){
    EmitMux *e=(EmitMux*)ud; mux_data(e->T,e->id,t);
}

static void run_serve_mux(Model *m, const char *snap){
    char tkp[2048]; snprintf(tkp,sizeof(tkp),"%s/tokenizer.json",snap);
    Tok T; tok_load(&T,tkp);
    int eos=tok_id_of(&T,"<|endoftext|>"); if(eos<0) eos=tok_id_of(&T,"<|end|>");
    if(g_temp<0) g_temp=0.7f;
    int maxctx=getenv("CTX")?atoi(getenv("CTX")):4096;
    int nslot=getenv("KV_SLOTS")?atoi(getenv("KV_SLOTS")):1;
    if(nslot!=1) fprintf(stderr,
        "[SERVE] hy3 engine supports a single KV slot; KV_SLOTS=%d requested, using 1\n",nslot);
#ifdef _WIN32
    _setmode(_fileno(stdin),  _O_BINARY);
    _setmode(_fileno(stdout), _O_BINARY);
    setvbuf(stdout, NULL, _IONBF, 0);
#endif
    int *hist=malloc((size_t)maxctx*sizeof(int)); int len=0;
    if(!hist){ fprintf(stderr,"OOM run_serve_mux hist\n"); exit(1); }
    kv_alloc(m,maxctx);
    printf("\x01\x01" "READY" "\x01\x01\n");
    printf("STAT 0 0.00 0.0 %.2f\n",rss_gb()); fflush(stdout);
    hwinfo_emit(m); tiers_emit(m); emap_emit(m);
    int busy=0;                      /* single slot: 1 while a request is mid-flight */
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(!strncmp(line,"CANCEL ",7)){
            ...
            /* nothing runs concurrently with getline(): by the time we read this
             * line any prior request has already finished and sent its DONE. */
        ...
        ColiSubmit sub;
        if(!coli_submit_parse(line,&sub)){ printf("ERROR 0 BAD_REQUEST\n"); fflush(stdout); continue; }
        char *raw=malloc((size_t)sub.bytes+1);
        if(!raw){ fprintf(stderr,"OOM mux payload\n"); exit(1); }
        int shortread = fread(raw,1,(size_t)sub.bytes,stdin)!=(size_t)sub.bytes;
        int delim=shortread?-1:fgetc(stdin);
        if(shortread || delim!='\n'){ free(raw); break; }   /* malformed frame: give up like EOF */
        raw[sub.bytes]=0;
        if(sub.slot!=0 || memchr(raw,0,(size_t)sub.bytes)){
            printf("ERROR %llu BAD_REQUEST\n",sub.id); fflush(stdout); free(raw); continue;
        }
        if(busy){ printf("ERROR %llu SLOT_BUSY\n",sub.id); fflush(stdout); free(raw); continue; }
        busy=1;
        int *tmp=malloc((size_t)maxctx*sizeof(int));
        if(!tmp){ fprintf(stderr,"OOM mux tmp\n"); free(raw); exit(1); }
        int nt=tok_encode(&T,raw,(int)sub.bytes,tmp,maxctx-2);
        free(raw);
        if(nt<1){ free(tmp); printf("ERROR %llu EMPTY_PROMPT\n",sub.id); fflush(stdout); busy=0; continue; }
        int prefix=0; while(prefix<len && prefix<nt && hist[prefix]==tmp[prefix]) prefix++;
        if(prefix<len){ len=prefix; if(m->has_mtp) m->kv_start[m->c.n_layers]=-1; }
        int add=nt-len;
        if(add>0) memcpy(hist+len,tmp+len,(size_t)add*sizeof(int));
        fprintf(stderr,"[API] KV prefix %d/%d token, prefill %d\n",len,nt,add);
        free(tmp);
        uint64_t h0=m->hits, ms0=m->miss; double t0=now_s();
        float *logit = add>0 ? step(m,hist+len,add,len) : step(m,hist+len-1,1,len-1);
        len+=add;
        int maximum=sub.max_tokens, room=maxctx-len-1, length_limited=0;
        if(maximum>room){ maximum=room; length_limited=1; }
        g_temp=sub.temperature; g_nuc=sub.top_p;
        int emitted=0;
        if(maximum>0){
            EmitMux em={&T,sub.id};
            emitted=spec_decode(m,hist,len,maximum,eos,logit,emit_mux,&em,NULL);
            len+=emitted;
        } else free(logit);
        double dt=now_s()-t0; if(dt<1e-6) dt=1e-6;
        double dh=(double)(m->hits-h0), dm=(double)(m->miss-ms0);
        usage_save(m); hwinfo_emit(m); tiers_emit(m); emap_emit(m); hits_emit(m);
        printf("DONE %llu STAT %d %.2f %.1f %.2f %d %d\n",sub.id,emitted,
               emitted/dt,(dh+dm)>0?100.0*dh/(dh+dm):0.0,rss_gb(),nt,length_limited);
        fflush(stdout);
        busy=0;
    }
    free(line); free(hist);
...
static void run_serve(Model *m, const char *snap){
     ...
     * sentinel never matches and chat hangs forever waiting for a byte
     * sequence that will never arrive, even though the engine already sent it
     * and is idle, blocked on stdin), and stdin translates '\r\n'->'\n' and
     * rejects writes of raw bytes with EINVAL, breaking the control protocol.
     * Put BOTH handles in BINARY mode so the protocol bytes are exact in both
     * directions (mirrors glm.c's run_serve/run_serve_mux fix, #195). */
#ifdef _WIN32
    _setmode(_fileno(stdin),  _O_BINARY);
    _setmode(_fileno(stdout), _O_BINARY);
    setvbuf(stdout, NULL, _IONBF, 0);
...
int main(int argc, char **argv){
    ...
    g_pilot_real=getenv("PILOT_REAL")?atoi(getenv("PILOT_REAL")):0; /* default OFF: real cross-layer loads; opt-in */
    ...
    if(g_pilot_real) fprintf(stderr,"[PILOT_REAL] cross-layer speculative expert prefetch enabled, PILOT_K=%d\n",g_pilot_k);
    else if(g_pilot) fprintf(stderr,"[PILOT] router-guided prefetch (WILLNEED-only) enabled, PILOT_K=%d\n",g_pilot_k);
