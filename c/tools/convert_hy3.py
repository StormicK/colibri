...
Fork of convert_fp8_to_int4.py with Hy3 tensor naming:
  - num_experts MoE layers, shared_mlp -> shared_experts (hy3.c tries both names)
  - MTP layer (index n_layers) -> out-mtp-*.safetensors (--mtp, int8)
  - no DSA indexer (Hy3 uses plain GQA attention, not MLA+indexer)

classify()/dequant() here are patched onto convert_fp8_to_int4 (cvt.classify /
cvt.dequant) for the real --repo download path, so their signatures must match
convert_fp8_to_int4.convert_shard()'s call sites exactly:
  classify(name, n_layers, keep_mtp=False, keep_idx=False)
  dequant(f, name, keys)
...
# Reuse quant + download machinery from the GLM converter.
...
def rename_out(name):
    """Colibri loader accepts either name (qt_load_first tries shared_experts first,
    falls back to shared_mlp) — renaming is optional, kept for GLM-style tooling parity."""
    return SHARED_RE.sub(".mlp.shared_experts.", name)


def classify(name, n_layers, keep_mtp=False, keep_idx=False):
    # keep_idx is unused: Hy3 has no DSA lightning indexer (plain GQA attention).
    del keep_idx
    if name.endswith("_scale_inv") or name.endswith(("_scale", "_scale_2")):
        return "consumed"
    li = layer_idx(name)
    if keep_mtp:
        if li != n_layers:
...
def convert_local(indir, outdir, n_layers, ebits, io_bits, xbits, keep_mtp=False):
    from safetensors.numpy import save_file
    shards = sorted(glob.glob(os.path.join(indir, "*.safetensors")))
    os.makedirs(outdir, exist_ok=True)
    for i, sp in enumerate(shards):
        out = {}
        convert_shard(sp, out, n_layers, ebits, io_bits, xbits, keep_mtp)
...
def main():
    ...
    if a.indir:
        ...
        convert_local(a.indir, a.outdir, a.n_layers, a.ebits, a.io_bits, a.xbits, a.mtp)
        return

    if not a.outdir:
        sys.exit("--outdir required")

    # Delegate to the GLM converter's real download/convert/delete loop, with
    # Hy3 classify()/dequant() patched in (signatures match convert_shard's call sites).
    import convert_fp8_to_int4 as cvt
    cvt.classify = classify
    cvt.dequant = dequant
    argv = [
        "convert_hy3.py",
        "--repo", a.repo,
